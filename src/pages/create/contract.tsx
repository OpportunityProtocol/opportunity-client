import { ChangeEvent, KeyboardEventHandler, ReactNode, useEffect, useState } from "react";
...
const CreateContractPage: NextPage = (): JSX.Element => {
  ...
  const [createContractForm, setCreateContractForm] = useState<any>({
    contract_title: "",
    ...
    marketsQuery.refetch()
      .then((value) => {
        if (!marketsQuery.error) {
          setMarketDetails(value.data?.markets)
        }
      })
      .catch(error => { })
  }, []);
