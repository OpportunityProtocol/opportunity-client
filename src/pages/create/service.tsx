...
import { NETWORK_MANAGER_ADDRESS, SERVICE_COLLECT_MODULE } from "../../constant";
...
const CreateServicePage: NextPage<any, any> = (): JSX.Element => {
  ...
  const networkManager_createService = useContractWrite(
    ...
    'createService',
    {
      ...
      args: [ 1, serviceMetadataKey, [Number(createServiceForm.offers.beginner.price), Number(createServiceForm.offers.business.price), Number(createServiceForm.offers.enterprise.price)], 0, SERVICE_COLLECT_MODULE],
      overrides: {
  ...
  const handleOnPublish = async () => {
    ...
    try { 
      let retVal;
      console.log(process.env.NEXT_PUBLIC_CHAIN_ENV)
      if (process.env.NEXT_PUBLIC_CHAIN_ENV === 'development') {
        ...
        retVal = await fleek.uploadService(String(accountData.data.address)+":"+createServiceForm.serviceTitle, JSON.stringify(createServiceForm))
      }
      
      setServiceMetadataKey(retVal)
      const confirm = await networkManager_createService.write({
        args: [1, retVal, [Number(createServiceForm.offers.beginner.price), Number(createServiceForm.offers.business.price), Number(createServiceForm.offers.enterprise.price)], 0, SERVICE_COLLECT_MODULE],
        overrides: {
          gasLimit: BigNumber.from('11643163')
        }
      })
  ...
  const handleOnChangeOffer = (e: ChangeEvent<HTMLInputElement | HTMLTextAreaElement>, idx: number) => {
    ...
    const enterpriseValues = createServiceForm.offers.enterprise.values.slice(0)
  ...
  const handleOnChangePrice = (e: ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
  ...
  return (
    <Container
      ...
      spacing={5}
      maxWidth="lg"
      sx={{ border: '1px solid #ddd', bgcolor: "#fff", padding: "2% 4% !important" }}
    >
      ...
      <Box>
        ...
        <Box  sx={{ width: "100%", border: "dashed 1px #B3B3B3", padding: 10, height: 400 }}>
          { selectedImage === null ?
          <Fragment>
            <input ref={fileRef} type='file'  onChange={handleOnChangeFile} style={{display: 'none'}} />
          <Stack alignItems="center">
            ...
            <Box
              ...
              alignItems="center"
            >
              <Typography  py={1} fontSize={25} variant='button' color='primary' onClick={() => fileRef.current.click()}>
          ...
          <Box sx={{ width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
             <img style={{ width: 200, height: 200 }} src={URL.createObjectURL(selectedImage)} />
          </Box>
          }
        </Box>
      </Box>
