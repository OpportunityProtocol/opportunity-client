...
import {
  IOpportunityProps,
  IContractChatsContentProps,
  IMarketDrawerContentProps,
...
const Opportunity: React.FunctionComponent<IOpportunityProps> = ({ children }) => {
  ...
  const isDrawerShowing =
    router.pathname.includes('create') ||
    router.pathname.includes('dashboard') ||
    router.pathname.includes('portfolio') ||
    router.pathname == '/contract/view'
      ? false
      : true;
...
const MarketDrawerContent = ({ classes }: IMarketDrawerContentProps) => (
  <Box py={2}>
    ...
    <FormControl sx={{ my: 2 }}>
      <FormLabel id="content-type-form-label" sx={{ fontSize: 13, fontWeight: 'bold' }}>
        Market Filters
      </FormLabel>
      <RadioGroup
        ...
        return (
          <Link href="/contract">
            <ListItemButton component={ListItem}>
              <ListItemAvatar>
                <Blockies seed={Math.random().toString()} size={10} scale={3} />
              </ListItemAvatar>
              <ListItemText
                primary="Elijah Hampton"
                secondary={
                  <React.Fragment>
                    <Typography
                      noWrap
